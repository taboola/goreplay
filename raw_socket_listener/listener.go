...
func (t *Listener) dispatchMessage(message *TCPMessage) {
	...
	if message.IsIncoming {
		...
		// log.Println("Removing alias:", message.Ack, t.respWithoutReq)
...
func (t *Listener) processTCPPacket(packet *TCPPacket) {
	...
	if parentAck, ok := t.seqWithData[packet.Seq]; ok {
		...
		log.Println("Adding ack alias", packet.Ack, parentAck)
		packet.Ack = parentAck
		delete(t.seqWithData, packet.Seq)
	}

	if alias, ok := t.ackAliases[packet.Ack]; ok {
		log.Println("Using ack alisas", packet.Ack, alias)
		packet.Ack = alias
	}
	...
	if len(packet.Data) >= 20 {
		log.Println("New packet:", packet.Seq, packet.Ack, string(packet.Data[:20]))
	} else {
		log.Println("New packet:", packet.Seq, packet.Ack, string(packet.Data))
	}
	...
	if !ok {
		...
		if !isIncoming {
			if responseRequest != nil {
				...
				log.Println("Can't find request", packet.Ack,  t.respAliases)
				t.respWithoutReq[packet.Ack] = mID
			}
		}

		if len(packet.Data) >= 20 {
			log.Println("New message:", message, string(packet.Data[:20]))
		} else {
			log.Println("New message:", message, string(packet.Data))
		}
	}
	...
	if isIncoming {
		...
		responseAck := packet.Seq + uint32(message.Size()) + uint32(len(packet.Data))
		t.respAliases[responseAck] = &request{message.Start, message.Ack}

		log.Println("ADDING RESP ACK", responseAck, message.Ack)
